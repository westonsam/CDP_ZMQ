...
#include <random> //used for captive portal message example
#include <iostream>
#include "json.hpp"
...
int main()
{
    ...
    pusher.bind("tcp://127.0.0.1:5555");
    receiver.bind("tcp://127.0.0.1:5556");
    zmq::pollitem_t items[] = {
        {static_cast<void *>(receiver), 0, ZMQ_POLLIN, 0}};
    //iter keeps track of # of iterations (used in random number gen. to create random DUID with a number under 1000). ONLY USED FOR DEMONSTRATION PURPOSES
    size_t iter = 0;
    ...
    for (;;)
    {
        ...
        if ((time(NULL) - lastMsg) > dist(rng))
        {
            ...
            jmsg["TOPIC"] = "cpm";
            jmsg["DATA"] = "Outgoing captive portal message";
            std::cout << "\nSENDING:\n"
